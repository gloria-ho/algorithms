...
// E.g. int*p = &val will assign the memory address of val to pointer p. To access the content
// of the memory to which the pointer points, prepend it with a *. For example, *p will return
// the value reflected by val and any modification to it will be reflected at the source (val).
